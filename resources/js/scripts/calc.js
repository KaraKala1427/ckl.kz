...
if (val<6){$("#k1-num-6").val('');$("#calc-param-k1-num").val(val);}
else {
...
function initk1num(){
if ($("#calc-param-k1-num").val()==''){return;}
...
else {$("#k1-num"+$("#calc-param-k1-num").val()).prop('checked', true);}
                ...
                success: function(a) {
                    ...
                    $('#ForerrorResult').show();
                    	//alert(a.errorResult);
					                      ...
					                      else { $("#forerror").html("");  $('#ForerrorResult').hide();}
...
if (validateEmail($(this).val())){$(this).closest('fieldset').addClass('has-success');}
else {$(this).closest('fieldset').removeClass('has-success');}

});
   
  
if (validateEmail($('input[name="order[email]"]').val())){$('input[name="order[email]"]').closest('fieldset').addClass('has-success');}
else {$('input[name="order[email]"]').closest('fieldset').removeClass('has-success');
...
if ($(this).val().length==10){$(this).closest('fieldset').addClass('has-success');}
else {$(this).closest('fieldset').removeClass('has-success');}
});	
       	...
       	c.additionalOptions(e);
            ...
            if (d.hasClass("vehiclerow")){
               vechcount++;
                 ...
                 //addvehicle
if ($('.vehiclerow').length==vechcount){$("#addvehicle").hide();}
...
if ($('.driverrow').length==drivercount){$("#adddriver").hide();}
...
if  (e.is("input[type=radio]") && e.val()=='v0-empty') {e.prop("checked", true);    }
else {
if (e.is("input[type=radio]")){e.prop("checked", false); }
            	  ...
            	  var ec = a(this);	c.hilitefieds(ec, !0) ;
...
if ($(this).val().length==10){$(this).closest('fieldset').addClass('has-success');}
...
}, a.fn.calcFormTumar = function() {  var ec = a(this);	c.hilitefieds(ec, !0) ;
            b = this, this.calcForm().checkForm(), this.calcForm().initDefault(), c.initDefault()
        }
    }(jQuery),
    function(a) {
	...
	else {	$("#k14-payment-numv1-1").prop("checked", true);
...
if(intval($("#calc-param-p0-sum").val())>0){$("#calc-param-p0-sum").closest('fieldset').addClass('has-success');}
else {$("#calc-param-p0-sum").closest('fieldset').removeClass('has-success');}

if(intval($("#calc-param-p3-year-full").val())>1980){$("#calc-param-p3-year-full").closest('fieldset').addClass('has-success');}
else {$("#calc-param-p3-year-full").closest('fieldset').removeClass('has-success');}

$("#calc-param-p0-sum").keyup(function(){

if(intval($("#calc-param-p0-sum").val())>0){$(this).closest('fieldset').addClass('has-success');}
else {$(this).closest('fieldset').removeClass('has-success');}
});
...
if(intval($("#calc-param-p3-year-full").val())>1980){$(this).closest('fieldset').addClass('has-success');}
else {$(this).closest('fieldset').removeClass('has-success');}
});
  ...
  // alert('kas');     
	 ...
	 c.hilitefieds(ec, !0) ;
        ...
        $("#p1-person-typev1-individual, #p1-person-typev2-legal").change(function(){ c.paymentNum()});
...
if(h){f.show();}	
            ...
            if (d === !0 ||   ( b.prop("id") &&  b.prop("id").match(/^k2-age-n/))) {
                ...
                h.each(function(){
if ($(this).prop('checked')){i=$(this).val();}
...
else {$("#calc-param-base-date-from").closest('fieldset').removeClass('has-success');
...
else {$("#calc-param-base-days").closest('fieldset').removeClass('has-success');
...
if ($(this).val().length==10){$(this).closest('fieldset').addClass('has-success');}
         ...
         var ec = a(this);
			 ...
			 $("#sendreq").on("click", c.calc);    c.hilitefieds(ec, !0) ;
            /*b.find("select").on("change", c.calc), b.find("input").smartKeyUp(c.calc), c.calc(!0)*/
...
if ($(this).is(":checked")){emfamily=1;}
}
);


if (emfamily==0){$("#k1-familyv0-empty").prop("checked", true);}
...
if ($(this).is(":checked")){emstom=1;}
}
);


if (emstom==0){$("#k3-stomatologyv0-empty").prop("checked", true);}
